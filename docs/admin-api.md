...
| `GET /api/calls/{id}/systemone` | Typed question/answer structure of one System One call, parsed from its capture: the state, the questions asked about it, and the answers. Read-only — it decodes the same captured bytes `/trace` returns. `404` when nothing was captured; an empty view (not an error) when the capture is not a System One shape. |
...
- **Daily burn and runway** on `/api/overview`. A budget is spent against the
  whole account; a runway derived from one model's slice of the burn would be a
  number that is true of nothing.
- **The facet lists** on `/api/usage/facets`. Cross-filtering them would make
  options vanish as you select, and can strand a selection you can no longer see
  to clear.
