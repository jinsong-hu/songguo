...
import type { ErrorCodeRow, UsageDimension, UsageFilter } from '../api/types';
...
import { brandOf, providerBrand, BrandIcon, ModelIcon, modelDisplayName } from '../lib/modelBrand';
...
export function OverviewPage() {
  ...
  // The filter as the client sends it, plus a scalar fingerprint of it. useFetch
  // spreads its deps into a useEffect array, so an array identity there would
  // refetch on every render — the joined form is what makes the dep stable.
  ...
  const tokenSeries = useFetch(
    () => api.tokensByModel(since, until, reqBucket, usageDim, filter),
    [since, until, reqBucket, usageDim, filterKey],
    opts,
  );
  ...
  const perfSeries = useFetch(
    () => api.tokensByModel(since, until, reqBucket, perfDim, filter),
    [since, until, reqBucket, perfDim, filterKey],
    opts,
  );
  const successSeries = useFetch(
    () => api.successByModel(since, until, reqBucket, successDim, filter),
    [since, until, reqBucket, successDim, filterKey],
    opts,
  );
  const cacheSeries = useFetch(
    () => api.cacheByModel(since, until, reqBucket, cacheDim, filter),
    [since, until, reqBucket, cacheDim, filterKey],
    opts,
  );
  ...
  const composition = useFetch(
    () => api.contextComposition(since, until, filter),
    [since, until, filterKey],
    opts,
  );
  ...
  const errorCodes = useFetch(
    () => api.errorCodes(since, until, successDim, effectiveService ?? undefined, filter),
    [since, until, successDim, effectiveService, filterKey],
    opts,
  );
  // A selected key is meaningless once the range, dimension, or top-bar filter
  // changes (the key set differs), so clear the raw state. Deriving
  // effectiveService already keeps the fetch correct in the same render; this
  // just tidies the stored selection.
  useEffect(() => {
    setSelectedService(null);
  }, [successDim, range, filterKey]);
  ...
  // Top bar: what to look at, then when. All three filters read from the same
  // facets response, so they appear and populate together.
  //
  ...
  return (
    <Page title="Overview" actions={toolbar}>
      ...
      {/* The one section where the top-bar filter changes meaning rather than
          just narrowing: a session is a whole agent run, not divisible by model,
          so filtering picks the runs that *used* a selected model, provider or
          client and still reports each run in full. The hint says so when a
          filter is on. */}
      <SectionTitle
        name="Behavioral"
        hint={
          models.length + vendors.length + clients.length > 0
      ...
      <ActivityFeed since={since} until={until} filter={filter} interactive={!isUser} />
    </Page>
  );
}
