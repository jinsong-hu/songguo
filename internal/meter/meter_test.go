...
func TestClassify(t *testing.T) {
	tests := []struct {
		...
		path     string
		body     string
		wantMod  ledger.Modality
		wantModl string
	}{
		{"chat completions", "/v1/chat/completions", `{"model":"gpt-4o"}`, ledger.ModalityChat, "gpt-4o"},
		{"legacy completions", "/v1/completions", `{"model":"gpt-3.5-turbo-instruct"}`, ledger.ModalityChat, "gpt-3.5-turbo-instruct"},
		{"embeddings", "/v1/embeddings", `{"model":"text-embedding-3-small"}`, ledger.ModalityEmbedding, "text-embedding-3-small"},
		{"tts", "/v1/audio/speech", `{"model":"tts-1"}`, ledger.ModalityTTS, "tts-1"},
		{"stt transcriptions", "/v1/audio/transcriptions", `{"model":"whisper-1"}`, ledger.ModalitySTT, "whisper-1"},
		{"stt translations", "/v1/audio/translations", `{"model":"whisper-1"}`, ledger.ModalitySTT, "whisper-1"},
		{"image generations", "/v1/images/generations", `{"model":"dall-e-3"}`, ledger.ModalityImage, "dall-e-3"},
		{"image edits", "/v1/images/edits", `{"model":"dall-e-2"}`, ledger.ModalityImage, "dall-e-2"},
		{"unknown path", "/v1/moderations", `{"model":"omni-moderation"}`, ledger.ModalityUnknown, "omni-moderation"},
		{"case insensitive", "/V1/Chat/Completions", `{"model":"gpt-4o"}`, ledger.ModalityChat, "gpt-4o"},
		{"trailing slash and query", "/v1/chat/completions/?stream=true", `{"model":"gpt-4o"}`, ledger.ModalityChat, "gpt-4o"},
		{"non-json body modality only", "/v1/chat/completions", "not json at all", ledger.ModalityChat, ""},
		{"empty body", "/v1/embeddings", "", ledger.ModalityEmbedding, ""},
		{"json without model", "/v1/chat/completions", `{"messages":[]}`, ledger.ModalityChat, ""},
	}
