...
- **号池 routing** — multiple vendors per model and multiple credentials per vendor; health → sticky session → priority → weight. One attempt per request: the vendor's response — success or failure (`429`/`5xx`/transport error) — is surfaced verbatim. No per-call retry or failover.
- **Auto fallback, cross-request** — vendor-side failures demote a vendor for 30s so the **next** request routes elsewhere. Failures are graded: conclusive ones (connection refused, DNS NXDOMAIN, bad TLS cert) demote immediately; ambiguous ones (timeout, connection reset, `5xx`, `403`) need 3 in a row; a `429` demotes only the `(vendor, model)` pair, since providers meter per model tier; a `401` demotes every vendor sharing that credential, since a revoked key is dead on all of them. Caller-side 4xx and client aborts never count. Detection is passive (no probe traffic), and demotion never excludes — a cooling vendor still serves if nothing healthier is available. Live state at `GET /api/vendors` under `routing`.
...
- **the body's `model` string** — routed across every vendor that serves it (health → sticky → priority → weight; the top candidate is forwarded to, no per-call failover).
...
backend/
  ...
  internal/
    ...
    router/     号池 candidate selection: health, sticky, priority, weight
